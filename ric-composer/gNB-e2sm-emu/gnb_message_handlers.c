...
#define NORMAL_DL_MCS 20
#define LOW_BER_VALUE 0.05f
#define HIGH_BER_VALUE 0.20f
...
void initialize_ues_if_needed(){
    ...
    for (int ue=0;ue<CONNECTED_UES;ue++){
        connected_ue_list[ue].rnti = 0x4600 + ue + 1;
        connected_ue_list[ue].prop_1 = false;
        connected_ue_list[ue].prop_2 = -1;
        connected_ue_list[ue].dl_ber = HIGH_BER_VALUE;
        connected_ue_list[ue].dl_mcs = NORMAL_DL_MCS;
        connected_ue_list[ue].target_dl_mcs = NORMAL_DL_MCS;
        connected_ue_list[ue].report_low_ber = (ue % 2 == 0);
    }
    is_initialized = true;
}

void update_project2_measurements(){
    for(int ue=0; ue<CONNECTED_UES; ue++){
        // Toggle the BER flag so the xApp can demonstrate both control directions.
        connected_ue_list[ue].report_low_ber = !connected_ue_list[ue].report_low_ber;
        connected_ue_list[ue].dl_ber = connected_ue_list[ue].report_low_ber ? LOW_BER_VALUE : HIGH_BER_VALUE;
    }
}
...
void apply_properties_to_ue_list(UeListM* ue_list){
    ...
    for(int ue=0; ue<ue_list->n_ue_info; ue++){
        if(ue_list->ue_info[ue]->has_target_dl_mcs){
            set_ue_dl_mcs_control(ue_list->ue_info[ue]->rnti,
                                  ue_list->ue_info[ue]->target_dl_mcs);
        } else {
            // Keep the base demo properties available for compatibility.
...
void set_ue_dl_mcs_control(int rnti, int target_dl_mcs){
    bool rnti_not_found = true;
    for(int ue=0; ue<CONNECTED_UES; ue++) {
        if(connected_ue_list[ue].rnti == rnti){
            connected_ue_list[ue].target_dl_mcs = target_dl_mcs;
            connected_ue_list[ue].dl_mcs = target_dl_mcs;
            printf("Applied DL MCS target %d to RNTI %d\n", target_dl_mcs, rnti);
...
UeListM* build_ue_list_message(){
    ...
    for(int i = 0; i<CONNECTED_UES; i++){
        ...
        // Project 2 measurements and control state.
        ue_info_list[i]->has_dl_ber = 1;
        ue_info_list[i]->dl_ber = connected_ue_list[i].dl_ber;
        ue_info_list[i]->has_dl_mcs = 1;
        ue_info_list[i]->dl_mcs = connected_ue_list[i].dl_mcs;
        ue_info_list[i]->has_target_dl_mcs = 1;
        ue_info_list[i]->target_dl_mcs = connected_ue_list[i].target_dl_mcs;
